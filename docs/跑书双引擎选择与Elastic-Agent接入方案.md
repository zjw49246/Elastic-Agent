...
```text
 ...
 TaskService + PipelineService    ElasticBookProductionService
             |                           |
 AIRequestQueue + ai_service      Elastic-Agent Manager
             |                           |
  ...
  app/
    ...
    services/
      ...
      elastic_agent/
        ...
        client.py                      # 调用 Elastic-Agent Manager 的 HTTP client
...
这个 webhook 只接收 Elastic-Agent Manager 主动推送的轻量事件，例如状态、phase、session_id、OSS key；最终稿正文仍由 `oss_file_service.py` 从 OSS 读取。

新增文件职责：

| 文件 | 职责 |
| --- | --- |
| `backend/app/api/elastic_agent.py` | 定义 `POST /api/elastic-agent/webhook`，接收 Elastic-Agent Manager 回调，并调用 `webhook_service.py` |
...
| `backend/app/services/elastic_agent/client.py` | 封装本项目调用 Elastic-Agent Manager 的 HTTP 客户端，例如 `produce_book()`、`cancel_task()`、`retry_task()`、`send_chat()` |
| `backend/app/services/elastic_agent/book_production_service.py` | Elastic 跑书主业务服务。负责从 `Task + Book` 组装入参、提交 Elastic-Agent、创建/更新 `elastic_book_runs` |
| `backend/app/services/elastic_agent/webhook_service.py` | 处理 Elastic-Agent 回调。负责验签、幂等、保存事件、更新状态、完成后触发 OSS 读取和 AgentOutput 回灌 |
...
external_task_id                Elastic-Agent Manager 侧任务 ID，推荐直接使用 task_id 字符串
...
| `elastic_agent` | 调 Elastic Manager cancel | 调 Elastic Manager continue | 调 Elastic Manager retry |
...
当 `force_sync=true` 且 Worker 在线时，后端通知 Elastic Manager 立即同步目标文件，再从 OSS 读取。
...
2. 完成事件重复到达时，不重复创建 `AgentOutput`，改为 update。
3. webhook 不应长时间阻塞，下载大文件可放后台任务。

### 5.8 Elastic Manager 客户端封装
...
`ElasticAgentClient` 封装后端到 Manager 的调用：
...
2. 生成 `book_slug`。
3. 调用 Elastic Manager `/api/tasks/produce`。
4. 创建 `elastic_book_runs`。
5. webhook 完成时回灌 `AgentOutput`。

### 5.9 我们与 Elastic-Agent Manager 的接口契约

这部分是后端和 Elastic-Agent 的核心边界。整体链路是：

```text
我们的前端
  -> 我们的 backend 创建 Task
  -> 我们的 backend 调 Elastic-Agent Manager 提交做书
  -> Elastic-Agent Manager 分发 Worker 执行
  -> Worker 把 workspace / final manuscript / session 同步到 OSS
  -> Elastic-Agent Manager webhook 通知我们的 backend
...
我们调用 Elastic-Agent Manager 时使用 Bearer Token：
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `task_id` | 是 | 我们系统的 `tasks.id`，推荐用字符串传递，Elastic 内部也以它作为主键 |
...
| `metadata` | 否 | 书籍补充信息 |
| `callback.url` | 是 | Elastic 事件回调到我们的地址 |
| `oss.bucket` | 是 | Elastic 同步产物的 bucket |
...
Elastic-Agent 内部会根据 `external_task_id -> session_id -> worker_id` 路由到对应 Worker，执行 `claude --resume`。
...
修改完成后，Elastic-Agent 仍通过 webhook 通知我们，并把新的最终稿同步到同一个 OSS 任务目录。我们需要重新读取 manifest 和最终稿，更新 `AgentOutput(final_proofreading)`。

#### 5.9.6 Elastic-Agent 写入 OSS 的目录约定

Elastic-Agent 不把大文件通过 webhook 回传，只把文件同步到 OSS。推荐目录：
...
```json
{
  ...
  "bucket": "audio-book-echo-editor-sh-oss",
  "prefix": "elastic-agent/tasks/123/",
  "latest_session_id": "claude-session-id",
  "status": "running",
  "phase": "phase_06_draft",
  "updated_at": "2026-05-18T10:20:00Z",
  "files": [
    {
      "path": "workspace/manuscript_final.md",
...
Elastic-Agent webhook 不传大文本，只传状态、路径和元数据：
...
1. 验签。
2. 按 `event_id` 做幂等。
3. 更新 `elastic_book_runs`。
4. 读取 manifest。
5. 按最终稿优先级从 OSS 下载 markdown。
6. 写入或更新 `AgentOutput(agent_name="elastic_audiobook")`。
7. 写入或更新 `AgentOutput(agent_name="final_proofreading")`。
8. 设置 `Task.status=REVIEWING`、`Task.script_status=PENDING_REVIEW`、`Task.current_step=elastic_completed`。

#### 5.9.9 我们对外提供给前端的接口仍由自己后端代理

前端不要直接访问 Elastic-Agent，也不要直接访问 OSS。前端只调用我们的后端：
...
5. 必要时调用 Elastic-Agent 做 `force_sync`、chat、retry、cancel。

### 5.10 本项目侧接口与工程改造清单

本方案的主要改造对象仍然是本项目。Elastic-Agent 可以理解为一个外部执行服务，本项目新增一层适配，把它接入当前 `tasks`、`AgentOutput`、审核、TTS、BGM 流程。
...
| `/api/elastic-agent/webhook` | POST | 接收 Elastic-Agent 状态事件 | Elastic-Agent Manager |

说明：

1. 前端仍然只调用本项目后端接口。
2. 本项目后端再决定是读本地数据库、调 Elastic-Agent Manager，还是读 OSS。
...
#### 5.10.3 本项目调用 Elastic-Agent 的内部入参

本项目不需要把前端请求原样转给 Elastic-Agent。应由 `ElasticBookProductionService` 从数据库组装完整入参：
...
本项目不依赖 Elastic-Agent 返回正文。正文统一从 OSS 读取：
...
async def handle_webhook(event: ElasticAgentWebhookEvent) -> None:
    ...
    run = await update_elastic_book_run(event)
...
前端只调用本项目后端，不直接调用 Elastic-Agent，也不直接访问 OSS。
...
| 取消生产 | `/api/tasks/{task_id}/script-production/cancel` | POST | `reason` 可选 | 操作结果 | legacy 映射现有 abort；Elastic 调 Manager cancel |
| 续跑生产 | `/api/tasks/{task_id}/script-production/continue` | POST | `from_latest_state` 可选 | 操作结果 | legacy 映射现有 continue；Elastic 调 Manager continue |
| 重试生产 | `/api/tasks/{task_id}/script-production/retry` | POST | `from_phase`、`reason` | 操作结果 | legacy 映射现有 retry；Elastic 调 Manager retry |
...
| 发送修改指令 | `/api/tasks/{task_id}/script-production/chat` | POST | `message`、`idempotency_key` 可选 | `edit_run_id`、状态 | Elastic Manager chat |
...
| `elastic_agent_options.persona` | string | `nonfiction_default` | 传给 Elastic-Agent 的 Audiobook persona |
...
### 10.3 本项目提交给 Elastic-Agent 的输入

本项目后端从 `tasks` 和 `books` 组装请求，然后调用 Elastic-Agent Manager：

```text
POST {ELASTIC_AGENT_MANAGER_URL}/api/tasks/produce
```

本项目传给 Elastic-Agent 的请求体：
...
| `book_slug` | 后端生成 | 本地目录名，如 `{title_slug}-{task_id}` |
...
### 10.4 本项目调用 Elastic-Agent 的其他接口

| 本项目动作 | Elastic-Agent 接口 | 方法 | 本项目传什么 | Elastic 返回什么 |
| --- | --- | --- | --- | --- |
...
### 10.5 Elastic-Agent 输出如何获取

Elastic-Agent 不通过接口直接返回大文本。输出统一写 OSS，并通过 webhook 或 status 返回 OSS 指针。
...
| `task.production.cancelled` | 标记任务失败，错误信息写“用户取消” |
...
| 取消任务语义不一致 | MVP 映射为 `FAILED + 用户取消错误信息`，后续可新增 `CANCELLED` 状态 |
