...
AWS Worker launches also carry a minimal cloud-init shell payload that unmasks
and enables `ssh.service`. This makes the Manager's intended SSH bootstrap
boundary independent of whether an AMI copy preserved the OpenSSH unit's
enable state; host keys and the authorized launch key are still generated by
cloud-init, and port 22 remains restricted by the Worker security group.

S3-capable Worker images must preinstall the pinned AWS CLI v2 binary. Ubuntu
Noble does not provide an `awscli` APT candidate, so system bootstrap excludes
that logical dependency from `apt-get` and verifies `command -v aws` plus
