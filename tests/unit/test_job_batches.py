...
class TestJobBatchValidationAndPlan:
    def test_global_and_manifest_policy_limits_accept_500(
        ...
        monkeypatch.setenv("ELASTIC_AGENT_JOB_BATCH_MAX_ACTIVE_JOBS", "500")
    ...
    def test_global_limit_rejects_values_above_500(
        ...
        monkeypatch.setenv("ELASTIC_AGENT_JOB_BATCH_MAX_ACTIVE_JOBS", "501")
        with pytest.raises(
            ValueError,
            match="ELASTIC_AGENT_JOB_BATCH_MAX_ACTIVE_JOBS must be between 1 and 500",
        ):
            JobBatchLimits()
...
class TestJobBatchDurability:
    ...
    @pytest.mark.asyncio
    async def test_restart_replays_accepted_item_with_prepared_job_journal(
        self,
        tmp_path: Path,
    ):
        from elastic_agent.core.job_spec_store import update_job_state
        ...
        async with AsyncClient(
            ...
            submitted = await first_client.post(
                "/api/job-batches",
                json=manifest,
                headers={"Idempotency-Key": "restart-prepared-batch"},
            )
            assert submitted.status_code == 201
            job_batch_id = submitted.json()["job_batch_id"]
            await _wait_for(
                lambda: first_batch.started_names == ["restart-prepared"]
            )
            before = (
                await first_client.get(f"/api/job-batches/{job_batch_id}")
            ).json()
            prepared_job_id = before["items"][0]["job_id"]
            update_job_state(
                first_manager.config.registry.path,
                prepared_job_id,
                "prepared",
        ...
        async with AsyncClient(
            ...
            await _wait_for(
                lambda: second_batch.started_names == ["restart-prepared"]
            )
            recovered = (
                await second_client.get(f"/api/job-batches/{job_batch_id}")
            ).json()

        assert second_batch.started == [prepared_job_id]
        assert recovered["items"][0]["state"] == "accepted"
        assert recovered["items"][0]["job_id"] == prepared_job_id
        assert second_provider.create_calls == 0
        await _stop_job_batch_queue(second_manager)
        await second_manager.stop()
