...
- **Agent API 账号（当前仅 CloudRouter）**（`core/agent_api.py` + `core/cloudrouter.py` + `worker/agent_api.py`）：独立 0700/0600 store 保存 write-only Key；`AgentApiProviderAdapter` 是 Apex 等后续 provider 的扩展点，但默认 registry 只注册 CloudRouter，未知 provider fail closed。CloudRouter 固定 `console.cloudrouter.online`，Bearer `GET /v1/models` 将一把 Key 投影为 Claude/Codex 双账号能力，`GET /v1/usage` 做 60 秒额度 admission（401/403/已耗尽/已过期/无效 schema 不可用，瞬时未知不得复活 fingerprint-bound、跨重启的 last-known unavailable；runtime hard quota 也持久 bench，但正常成功 probe 可清，invalid-key/model tombstone 需显式 refresh；数值和嵌套展示字段有界 allowlist）。API 账号与 OAuth 账号共享 account_id claim/EIP namespace，fresh allocation API-first、不可用时回退 OAuth；显式 native/OAuth id 不受无关 API probe 阻塞，API usage probe 有并发上限和绝对 deadline；显式 `account.ids` 精确选号，可选 `account.model` 在 plan→claim→configure 三处校验具体模型（不改写 Mode B 命令本身）。Manager 只在 WSS 登录边界通过关联 `AGENT_API_CONFIGURE` 将 Key 一次性下发，Worker 将 Key 放私有文件并用 helper 供 Claude/Codex 取值；Key 不进 JobSpec、REST、CLI config、env 或 argv。投影 schema v2 增加 byte-exact 0700 launcher，以 `/usr/bin/env -i` 清空环境并固定系统 PATH 后再执行 helper，防止 Job 可写 PATH/PYTHONPATH 劫持取 Key 的 Python。Claude PTY 最终 wrapper 再次清全部官方 auth/provider env 并只加载 user settings（排除 project/local hooks/MCP），Codex 用 Responses custom provider；两者固定 endpoint 并拒绝 Job env/argv 重定向。CLI 结构化 provider failure 即使 OS exit 0 也转为 Job failure，多帧错误按 auth>硬限流/timeout>瞬时>generic 保留最可操作根因。普通 Worker 必须确认销毁后才释放 claim，EIP Worker 沿 durable cleanup；Manager 启动恢复未确认旧 Worker 云终态时暂停所有 API 账号选择/Key 下发（OAuth 仍可用）。EIP preflight 拒绝 `run.env`/`run.secret_env` 的 HTTP(S)/ALL proxy 路由变量，兼容容器 owner 还必须拒绝宿主/adapter proxy，保证 CloudRouter 观察到绑定 EIP 而非代理出口。Worker 仅对已验证投影强制导出非秘密 `ELASTIC_AGENT_API_PROJECTION_ROOT`，普通任务会清除此保留变量；容器 owner 必须校验 v2 marker/owner/mode/launcher/helper 后只读同路径挂载单一账号 root 并传递对应 CLI home，Elastic 不透明劫持任意 Docker。API 账号删除暂禁用。Job 代码与 helper/key 同属一个 Unix 用户，因此 API Key 是委托给该 Job 的凭据，只能运行可信代码。
- **Agent API 时限与换号边界**：Provider 单请求总时限 15 秒；自动整池 usage 刷新并发 16/总时限 30 秒，未完成 Key 本轮排除并可回退 OAuth；configure 的 usage→读 Key→Worker ACK 共用 60 秒总时限。显式 native/OAuth id 跳过无关 API probe。非 EIP 动态 API→OAuth 换号只能写 projection 外的 `<source-slot>-rot-N` sibling；无法从精确 `.elastic-agent-api/cloudrouter/<account>/<agent>` 后缀安全反推 source slot 时 fail closed。
