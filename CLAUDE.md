...
- **Manager 生产启动边界**：AWS 生产入口固定为 `deploy/aws_manager.py` + 版本化 `deploy/aws/elastic-agent-manager.service`。秘密与非秘密云配置分别由必需的 `/etc/elastic-agent-manager.env`、`/etc/elastic-agent-manager.aws.env` 注入；unit 将 release/HOME 设为只读，仅允许写生产 state 目录，并启用 `NoNewPrivileges`、`PrivateTmp` 和 kernel/control-group 防护。AMI provenance 校验失败时在 Manager 生命周期启动前 fail closed。
