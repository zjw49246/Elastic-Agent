...
- **声明式 JobSpec**（`core/job_spec.py`）：任务即数据，所有外部 section 均 `extra="forbid"`，拼错字段直接 422。`environment.profile` 选择版本化、不可变的通用环境（默认 `ubuntu-agent-v1`；Docker 版 `ubuntu-agent-docker-v1`），Job 增量写在 `setup`/`run`。`setup.commands` 旧字符串列表继续兼容；新 `setup.steps` 支持 `name/command/env/cwd/timeout/retries` 且固定以 Job 用户执行。代码可用 `ref`（branch/tag）并用完整 `resolved_commit` 直接 fetch 精确 immutable checkout，不受 mutable ref 后续前进影响。`run.timeout` 缺省/旧 0 归一为 24h，`ttl_seconds` 默认 48h，二者最大 30 天且 TTL 不得短于 run。其余 section：`account`(**agent_type**/mode/per_worker/group/config_dir/**binding/ids**)/`rotation`/`fanout`(workers/shard_by/name_prefix/**instance_type/region/disk_gb/spot**)/`collect`/`completion`。`agent_type="codex"` 不支持 `manager_distribute`，运行时注入 `CODEX_HOME`；Claude 注入 `CLAUDE_CONFIG_DIR`。模板 `{{shard_index}}`/零填充五位 `{{shard_id}}`/`{{num_shards}}`/`{{hostname}}` 由 Manager 渲染，适用于 run/env 和 `setup.s3_datasets[].uri/dest`；`$(hostname -s)`/`$VAR` 留给 worker shell。**per-job 机器覆盖**经 orchestrator→driver→Manager，但 Job region 当前必须与 Manager provider region 一致。
...
- **增量收集 & 结果落 S3**（`collect.paths` + `interval_seconds`）：只收显式 paths；空列表是 no-op（Batch UI 默认填 `results`，API/SDK 必须显式给），stdout/stderr 只是日志，不会自动成为 S3 对象。`interval_seconds>0` 在首个完整间隔后周期收集，运行中可下载最近一次已完成快照；`0`（Schema/UI 默认）只做终态收集，下载不会临时触发 Worker 同步。成功/失败/取消都会 awaited final collect。每 shard 独立落 `<prefix>/<job>/workers/shard-00000/<path>/` 并写 `_elastic_agent/collection.json`，避免同名覆盖。AWS worker profile+bucket 时 worker 用实例角色直推；否则 rsync `--safe-links` 回 Manager 后由 `S3ResultUploader` 上传；无桶仅留本地。上传按内容 SHA-256 判变，跳过 symlink/special file；结果 API S3 优先，校验 key/path，score 读取和 tar download 有对象数/总大小/单文件上限且对象在 LIST→GET 间变化时 fail closed。UI 使用独立的 S3 流式 tar 路由（有界 pipe、gzip level 1）提前返回数据、显示传输字节并支持取消，取消时关闭当前 S3 body；原严格下载路由仍先完整构建，以便响应头前返回对象读取错误。run 退出靠 `proc.returncode` 而非等管道 EOF，保证 final collect 可触发。
- **可靠终态、取消与清理**：`PROCESS_EXIT`/`RUN_EXHAUSTED` 带 UUID event_id，worker 在 0600 fsync outbox 中按产生顺序保存，Manager 处理成功才 ACK；重连先 replay 再 STATUS，退出中 task 也进入快照，避免误判。同一 event_id 的 in-flight replay 只由一个 handler 执行，其余等待：成功后去重 ACK，失败/取消则不标 processed，由等待者接手或主动断开 WS 触发 worker 重连重放。若终态 handler 已在清理中移除/替换并关闭当前 WS，Manager 保留 processed event_id 但不向旧连接强发 ACK；短暂重连的 replay 由新连接 ACK，活动连接的真实发送错误仍向上抛出。PTY STOP/launch cancel/shutdown 共用 exactly-once 独立 finalizer，底层 teardown 抛错也必须先 mark exiting 并可靠发终态；Mode B 则先终止整个 POSIX process group。取消会关联取消 bootstrap/login，已 dispatch 的命令先 TERM、超时 KILL、等可靠 exit，再 final collect 和销毁；普通 Job 默认也是临时 EC2，终态 force terminate 后从 task/Node registry 移除（不为每个 shard 永久积累 TERMINATED 记录），EIP Job detach/terminate 后保留 EIP。云创建/registry/WS/collect/终止各失败路径均有幂等后台补偿，Manager shutdown/restart 也扫描带 controller/job tag 的遗留实例。普通 Job 的 create→registry→event 发布与恢复 pass 共用生命周期锁；本进程活实例还须同时匹配内存 instance id、云端 controller/job tag 与 registry metadata，重启不信任旧内存所有权。普通 Job 每次进入云 create 前先在独立 0600 `unbound-launches.json` 按 Job 计数并 fsync；完整 registry/event 发布后才把无 ID 意图转交给 durable NodeRecord，重启会对这些 exact instance id 同样保留完整可见性隔离。无 ID 意图只有匹配实例确认终止或完整窗口连续未发现时才减记/清除。因此 `RunInstances` 已接受但 timeout/cancel 未返回 instance id、Job 已先写 terminal、随后 Manager 再崩溃时，下一进程仍会按 controller/job tag 重扫和销毁幽灵实例。
...
- **S3 数据集分发**（`setup.s3_datasets: [{uri,dest}]`）：**worker 直连 S3 拉**（不再经 Manager 中转）。前提 `fanout` provider 配了 `worker_instance_profile`（AWS：`AWSProviderConfig.worker_instance_profile`，`create_instance` 给 `run_instances` 加 `IamInstanceProfile`；Manager 角色需 `iam:PassRole`）→ worker EC2 挂 IAM 角色（`elastic-agent-worker`/S3 权限）。provision hook 先用该 worker 的 context 渲染 uri/dest，再在 run 前装 awscli（缺才装）→ 逐个 `aws s3 sync <uri>/ <dest>`（尾 `/`=前缀递归）或 `aws s3 cp`（单对象）在 worker 上跑；正式 fanout 应优先用 `shard-{{shard_id}}` 单对象，避免每台 worker 重复拉完整 prefix。未知模板变量在 JobSpec 校验时 fail closed。**GitHub 代码分发不变**（仍 manager_rsync，token 只在 Manager）。旧的 `code_sync.py` `_download_s3`/`ManagerCodeSync.stage_s3`（Manager 下载→rsync）保留为工具函数但 provision 不再用。凭证仍不下发（worker 用实例角色，非静态 key）。
...
- **前端与绑定管理 API**（`api/routes/`）：`/api/accounts` 返回统一 OAuth/API 账号池；OAuth 密码/邮箱查询 token 与 `/api/agent-api/accounts` 的 CloudRouter/ApexRouter Key 都 write-only，API 只返回存在标志、支持 Agent/模型和缓存额度。Agent API 表单可选择 provider，提交后立即清空 Key；ApexRouter 在 UI/plan 中明确限制为 Codex。API 账号只允许刷新模型/额度、暂不允许删除；Job picker 按每个 identity 的实际 Agent 能力过滤。Job 提交表单按基本信息/计算资源/代码初始化/账号/运行/结果/换号/Harness 八段组织，保留全部既有控件 ID 与 JobSpec 映射；低频字段折叠，结果路径与增量收集始终显眼，不适用的 Repo/账号/EIP/换号控件会禁用并显示原因，桌面端操作栏保持可达、窄屏改为全宽按钮。提交前前端校验必填命令、数值边界、TTL 与运行超时关系及 S3 数据集行格式。管理 API key 仅接受 Bearer/X-API-Key header，不接受 query；UI 只存 sessionStorage 并清理旧 query。OTP 只在活跃人工 challenge 存在时显示：每个 Worker 在所属 Job 内有独立卡片，明确账号邮箱/ID、完整 Worker、Job 与 shard；新 challenge 首次展开对应 Job，浮动提醒可跳转并在移动端收成小条，多个 challenge 互不遮挡。OTP DOM 以 request+challenge 为 key，Job 轮询替换会移植原节点并恢复输入、焦点和选区；验证码不进入 JS 持久状态或日志。绑定 decommission 双确认且是唯一释放 EIP 入口，active claim/lease 时拒绝；Batch UI 删除有绑定的 OAuth 账号时先显示具体 EIP 和不可恢复警告、要求输入完整账号 ID，decommission 成功后才删 identity，任何 409 都保持未完成状态；账号 CRUD/ensure 与 Job claim 串行。Batch/Fleet 默认浅色、深色仅 session 级可选；Jobs/OTP/Nodes 用 keyed reconcile + 不重叠且隐藏页暂停的轮询，避免全量 DOM 重建、焦点/展开/日志滚动丢失及 results 请求风暴。Results 查询用 per-Job 请求版本拒绝乱序响应，已知非空结果不会被空值/错误回退，终态空结果仍有限退避复查；结果操作节点始终保留并区分检查中、暂无、暂不可用、可下载与下载中，单 Job 只允许一个下载请求。每张 Job 卡默认折叠，只在摘要展示状态，用户展开后查看操作、错误、结果和 Worker 明细；轮询替换节点时保留用户的展开、焦点、视口和表格横向滚动。Worker 尚未释放时，即使执行已终态也保留只读 systemd 日志入口，资源消失返回 404/409 后停止自动重试。Batch UI 在 AWS Manager 上默认选择 EIP 模式并展示持久 EIP；显式 `binding=none` 仍受支持，但 plan 会警告它绕过账号 EIP、使用临时公网出口。`/api/jobs` 提供 plan/submit/idempotency/cancel/results/logs，Harness 上传默认关闭。
