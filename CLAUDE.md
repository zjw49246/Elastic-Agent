...
- **Reconciler 终态守卫**：云 API 会在 EC2 已终止后继续返回历史行；registry 不存在时，terminated **无 lease** orphan 必须忽略，不能重新收养并累积 Node；带 lease 的 terminated orphan 仍必须收养并触发 durable EIP cleanup。
