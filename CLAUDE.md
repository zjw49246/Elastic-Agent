...
- **S3 数据集分发**（`setup.s3_datasets: [{uri,dest}]`）：worker **没 IAM/S3 凭证**（结果上传也是 worker→Manager→S3，Manager 侧 boto3），所以数据集也走 **Manager 下载→rsync**：`code_sync.py` `_download_s3`（boto3，prefix 尾 `/`=递归）+ `ManagerCodeSync.stage_s3` 复用 `deliver` rsync 到 worker `dest`；provision hook 在 run 前逐个 stage。凭证不上 worker。
